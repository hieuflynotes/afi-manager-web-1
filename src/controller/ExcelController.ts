...
export class ExcelController {
    ...
    runBotCheckout(orderId?: string, email?: string, quantity?: number, warehouse?: string): Promise<any> {
        return this.getConfigList().then(rst =>
            this.client.post(`${config.apiProxyUrl}/proxy?url=${rst.bot.apiGatewayUrl}&action=create-order&orderId=${orderId}&warehouse=${warehouse}&email=${email}&quantity=${quantity}`)
                .then(res => res.data))
    }
}